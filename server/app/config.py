...
class Settings(BaseSettings):
    ...
    # --- Budgets (journey-bound, spec A.6.2) ---------------------------------
    ...
    # implemented (see docs/BUILD.md). Scheduling records only — no float.
    # Defaults to FALSE (fail-closed; P3 C-M6): flip explicitly to enable.
    ...
    # --- Receipt TTL -----------------------------------------------------------
    receipt_ttl_seconds: int = 30 * 24 * 3600  # 30 days (spec A.2)

    # --- Merchant auth ---------------------------------------------------------
    # Optional gate on /v1/conversions: if set, require X-Crumbs-Key header.
    # Real per-merchant keyed tokens are a POST-v0.1 item (see docs/BUILD.md).
