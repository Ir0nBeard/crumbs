...
| **Python x402 kit** | Zero-dependency server-side helpers for x402 sellers: `x402_referral_field()` (PAYMENT-RESPONSE referral), `builder_code()` (`bc_crumbs`), did:pkh agent anchors, thin ledger client (`request_journey`, `verify_receipt`). Mirrors the JS SDK carrier semantics | `sdk/python/` |
